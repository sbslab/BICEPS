...
model HeatPumpAntoine "Base subsystem with water-to-water heat pump"
  ...
  Buildings.Fluid.Sensors.TemperatureTwoPort senTConLvg(
    ...
    final m_flow_nominal=m1_flow_nominal) "Condenser water leaving temperature"
    annotation (Placement(transformation(
        extent={{10,-10},{-10,10}},
        rotation=-90,
        origin={40,20})));
...
protected
  Buildings.Fluid.FixedResistances.LosslessPipe pip(redeclare package Medium =
        Medium1, m_flow_nominal=m1_flow_nominal) if not have_pumCon
